...
public class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        var subscriptions = string.Join(
            ", ",
            _productionFlow.GetSubscriptions(AgentRoles.Storyteller).Select(subscription => subscription.EventType));
        ...
        {
            _logger.LogInformation("{AgentRole} stopped.", AgentRoles.Storyteller);
        }
    }
}
